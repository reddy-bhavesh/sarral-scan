...
class WebScraperRecon:
    ...
    def run_http_probe(self):
        ...
        for url in candidates:
            try:
                ...
                # Pass r.headers (CaseInsensitiveDict) to helpers to ensure case-insensitive lookups
                self.security_headers = self._build_security_headers(r.headers)

                body = r.text[:50000] if "text/html" in self.headers.get("Content-Type", "") else ""
                self._fingerprint(r.headers, body)
    ...
    def extract_from_text(self, text: str):
        ...
        for email in found_emails:
            if any(email.lower().endswith(ext) for ext in SKIP_EXTENSIONS):
                continue
        ...
        # Social Profiles
        found_social = SOCIAL_URL_RE.findall(text)
        for url in found_social:
            # Filter out generic homepages
            # e.g. https://facebook.com or https://facebook.com/
            path = urlparse(url).path
            if path in ["", "/"]:
                continue
            self.social_profiles.add(url)
    ...
    def crawl(self) -> ScrapeResult:
        ...
        # If probe web only, we are done after probe
        if self.probe_web:
            return self._build_result(time.time() - start)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of concurrent threads for multi-domain scanning (default: 10).",
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument(
        "--web-hosts-output",
        help="Path to write valid, live URLs (web_hosts.txt) for downstream tools.",
    )
    parser.add_argument(
        "--stdout",
        action="store_true",
        help="Print JSON output to stdout (suppresses other output).",
    )
    parser.add_argument(
        "--only-dns",
        action="store_true",
        help="Only resolve IPs and exit (skips HTTP probing and crawling).",
    )
    parser.add_argument(
        "--probe-web",
        action="store_true",
        help="Only probe for alive web hosts and exit (skips crawling).",
    )
...
def run_single_target(target: str, args, output_dir: str) -> Dict[str, Any]:
    ...
    scraper = WebScraperRecon(
        ...
        only_dns=args.only_dns,
        probe_web=args.probe_web,
    )

    result = scraper.crawl()
    data = asdict(result)

    # Only write individual files if NOT in stdout mode (to keep it clean)
...
def main():
    ...
    if args.list:
        ...
        if not args.stdout:
            print(f"[+] Loaded {len(targets)} unique targets from {list_path}")
            print(f"[+] Starting scan with {args.concurrency} threads...")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrency) as executor:
            future_to_target = {
                executor.submit(run_single_target, t, args, base_output_dir): t 
                for t in targets
            }
            ...
            for future in concurrent.futures.as_completed(future_to_target):
                ...
                try:
                    ...
                    # Collect live URLs for web_hosts.txt
                    if data.get("alive") and data.get("http_probe", {}).get("final_url"):
                        live_urls.append(data["http_probe"]["final_url"])
        ...
        if args.combined_output and not args.stdout:
            with open(args.combined_output, "w", encoding="utf-8") as cf:
        ...
        # Write web_hosts.txt if requested
        if args.web_hosts_output:
            try:
                with open(args.web_hosts_output, "w", encoding="utf-8") as wh:
                    for url in live_urls:
                        wh.write(url + "\n")
                if not args.stdout:
                    print(f"[+] Live web hosts saved to: {args.web_hosts_output}")
            except Exception as e:
                if not args.stdout:
                    print(f"[!] Failed to write web_hosts.txt: {e}", file=sys.stderr)

        # STDOUT Output (Critical for ScanManager)
        if args.stdout:
            # If multiple targets, we output the combined dict
            print(json.dumps(combined, indent=2 if args.pretty else None, ensure_ascii=False))

    else:
        # Single target mode
        target = args.target
        data = run_single_target(target, args, base_output_dir)
